...
class PolicyRAGService:
    ...
    def __init__(self):
        ...
        if not LANGCHAIN_AVAILABLE:
            logger.error(
                "LangChain libraries not installed. Install with: pip install -r requirements_ai.txt"
            )
    ...
    def index_all_policies(self, policy_dir: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not policy_dir_path.exists():
                logger.error(f"Policy directory not found: {policy_dir_path}")
                return {"success": False, "error": "Directory not found"}

            policy_files = list(policy_dir_path.glob("*.pdf")) + list(
                policy_dir_path.glob("*.txt")
            )
            if not policy_files:
                logger.warning(f"No PDF or TXT files found in {policy_dir_path}")
                return {
                    "success": True,
                    "indexed": 0,
                    "message": "No policies to index",
                }
    ...
    def ask_question(
        ...
        try:
            ...
            response = self.chain.invoke(
                {"input": question, "chat_history": lc_chat_history}
            )
