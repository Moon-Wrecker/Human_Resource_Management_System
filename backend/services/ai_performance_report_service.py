...
from models import (
    User, UserRole, Goal, GoalStatus, GoalCheckpoint, GoalComment, GoalHistory,
    Feedback, Attendance, AttendanceStatus, SkillModuleEnrollment, ModuleStatus,
    PerformanceReport, Department, Team
)
from schemas.ai_performance_schemas import (
    TimePeriodEnum, ReportTemplateEnum, MetricEnum, DataSummary,
    AIReportResponse, TeamMemberReport, TeamReportResponse,
...
class AIPerformanceReportService:
    ...
    def aggregate_employee_performance_data(
        ...
        if "goal_completion" in selected_metrics or "overdue_goals" in selected_metrics or not selected_metrics:
            metrics_data.update(self._aggregate_goals_data(db, employee_id, start_date, end_date))
        
        # Aggregate feedback data
        if "feedback_ratings" in selected_metrics or "feedback_sentiment" in selected_metrics or not selected_metrics:
            metrics_data.update(self._aggregate_feedback_data(db, employee_id, start_date, end_date))
        
        # Aggregate attendance data
        if "attendance_rate" in selected_metrics or not selected_metrics:
            metrics_data.update(self._aggregate_attendance_data(db, employee_id, start_date, end_date))
        
        # Aggregate training data
        if "training_completion" in selected_metrics or "skills_development" in selected_metrics or not selected_metrics:
            metrics_data.update(self._aggregate_training_data(db, employee_id, start_date, end_date))
        
        # Aggregate collaboration data
        if "peer_collaboration" in selected_metrics or not selected_metrics:
            metrics_data.update(self._aggregate_collaboration_data(db, employee_id, start_date, end_date))
        
        # Add team comparison if requested
        if include_team_comparison and employee.team_id:
            metrics_data.update(self._add_team_comparison(db, employee, start_date, end_date, metrics_data))
        
        # Add period comparison if requested
        if include_period_comparison:
            metrics_data.update(self._add_period_comparison(db, employee_id, start_date, end_date, metrics_data))
    ...
    def _aggregate_goals_data(
        ...
        overdue_goals = sum(1 for g in goals if g.target_date < date.today() and g.status != GoalStatus.COMPLETED)
        ...
        completed_with_dates = [g for g in goals if g.status == GoalStatus.COMPLETED and g.completion_date]
        if completed_with_dates:
            completion_times = [(g.completion_date - g.start_date).days for g in completed_with_dates]
            avg_completion_days = sum(completion_times) / len(completion_times)
            
            # On-time completion rate
            on_time_completed = sum(1 for g in completed_with_dates if g.completion_date <= g.target_date)
        ...
        for priority in ["low", "medium", "high", "critical"]:
            ...
            if count > 0:
                completed = sum(1 for g in goals if g.priority == priority and g.status == GoalStatus.COMPLETED)
                goals_by_priority[priority.title()] = f"{completed}/{count} ({completed/count*100:.1f}%)"
        ...
        for cat_id in category_ids:
            ...
            if cat_goals:
                cat_name = cat_goals[0].category.name if cat_goals[0].category else f"Category {cat_id}"
                completed = sum(1 for g in cat_goals if g.status == GoalStatus.COMPLETED)
                goals_by_category[cat_name] = f"{completed}/{len(cat_goals)} ({completed/len(cat_goals)*100:.1f}%)"
        ...
        overdue_goal_examples = [
            ...
            for g in sorted(
                [g for g in goals if g.target_date < date.today() and g.status != GoalStatus.COMPLETED],
    ...
    def _aggregate_feedback_data(
        ...
        avg_rating = sum(f.rating for f in rated_feedback) / len(rated_feedback) if rated_feedback else 0
        
        # Count by type
        positive_count = sum(1 for f in feedbacks if f.feedback_type == "positive")
        constructive_count = sum(1 for f in feedbacks if f.feedback_type == "constructive")
        performance_count = sum(1 for f in feedbacks if f.feedback_type == "performance")
    ...
    def _aggregate_attendance_data(
        ...
        days_absent = sum(1 for a in attendance_records if a.status == AttendanceStatus.ABSENT)
        wfh_days = sum(1 for a in attendance_records if a.status == AttendanceStatus.WFH)
        
        # Calculate attendance rate (present + wfh as attended)
        attendance_rate = ((days_present + wfh_days) / total_days * 100) if total_days > 0 else 0
    ...
    def _aggregate_training_data(
        ...
        completed_enrollments = db.query(SkillModuleEnrollment).filter(
            ...
            SkillModuleEnrollment.completed_at >= datetime.combine(start_date, datetime.min.time()),
            SkillModuleEnrollment.completed_at <= datetime.combine(end_date, datetime.max.time())
        ).all()
        
        # Get all enrollments (for completion rate)
        all_enrollments = db.query(SkillModuleEnrollment).filter(
            SkillModuleEnrollment.employee_id == employee_id,
            SkillModuleEnrollment.enrolled_at >= datetime.combine(start_date, datetime.min.time())
        ).all()
        
        modules_completed = len(completed_enrollments)
        modules_in_progress = sum(1 for e in all_enrollments if e.status == ModuleStatus.IN_PROGRESS)
        total_enrolled = len(all_enrollments)
        
        training_completion_rate = (modules_completed / total_enrolled * 100) if total_enrolled > 0 else 0
    ...
    def _aggregate_collaboration_data(
        ...
        comments = db.query(GoalComment).join(Goal).filter(
            GoalComment.user_id == employee_id,
            GoalComment.created_at >= datetime.combine(start_date, datetime.min.time()),
            GoalComment.created_at <= datetime.combine(end_date, datetime.max.time()),
    ...
    def _add_team_comparison(
        ...
        for member in team_members:
            # Goal completion
            member_goals = db.query(Goal).filter(
            ...
            if member_feedback:
                team_ratings.append(sum(f.rating for f in member_feedback) / len(member_feedback))
            
            # Attendance
            member_attendance = db.query(Attendance).filter(
            ...
            if member_attendance:
                present = sum(1 for a in member_attendance if a.status in [AttendanceStatus.PRESENT, AttendanceStatus.WFH])
                team_attendance_rates.append(present / len(member_attendance) * 100)
        
        team_avg_completion = sum(team_completion_rates) / len(team_completion_rates) if team_completion_rates else 0
        team_avg_rating = sum(team_ratings) / len(team_ratings) if team_ratings else 0
        team_avg_attendance = sum(team_attendance_rates) / len(team_attendance_rates) if team_attendance_rates else 0
        ...
        return {
            ...
            "vs_team": employee_metrics.get("goal_completion_rate", 0) - team_avg_completion
        }
    ...
    def _add_period_comparison(
        ...
        prev_goals = db.query(Goal).filter(
            Goal.employee_id == employee_id,
        ...
        prev_feedback = db.query(Feedback).filter(
            Feedback.employee_id == employee_id,
            Feedback.given_on >= datetime.combine(prev_start_date, datetime.min.time()),
            Feedback.given_on <= datetime.combine(prev_end_date, datetime.max.time()),
            Feedback.rating.isnot(None)
        ).all()
        
        prev_rating = sum(f.rating for f in prev_feedback) / len(prev_feedback) if prev_feedback else 0
        
        return {
            "previous_period": True,
            "prev_completion_rate": prev_completion_rate,
            "prev_rating": prev_rating,
            "period_trend": current_metrics.get("goal_completion_rate", 0) - prev_completion_rate
        }
    ...
    def _assess_data_sufficiency(self, metrics: Dict[str, Any]) -> DataSummary:
        ...
        if data_points >= 50:
            ...
            sufficiency = "limited"
            warnings.append("Limited data available. Report may not capture full performance picture.")
        else:
            sufficiency = "insufficient"
            warnings.append("Insufficient data for comprehensive analysis. Consider longer time period or accumulate more data.")
        
        if total_goals < 3:
            warnings.append("Very few goals recorded in this period.")
        
        if total_feedback < 2:
            warnings.append("Limited feedback available. Encourage more regular feedback.")
    ...
    async def generate_individual_report(
        ...
        aggregated_data = self.aggregate_employee_performance_data(
            ...
            selected_metrics, include_team_comparison, include_period_comparison
        )
        ...
        response = AIReportResponse(
            ...
            metrics_used=[m.value if isinstance(m, MetricEnum) else m for m in selected_metrics],
        ...
        logger.info(f"Report generated successfully for employee {employee_id} in {generation_time:.2f}s")
    ...
    def _get_template_metrics(template: ReportTemplateEnum) -> List[str]:
        ...
        if template == ReportTemplateEnum.QUICK_SUMMARY:
            ...
            return [
                "goal_completion", "feedback_ratings", "attendance_rate",
                "training_completion", "overdue_goals"
            ]
        
        elif template == ReportTemplateEnum.COMPREHENSIVE_REVIEW:
            return [
                "goal_completion", "attendance_rate", "training_completion",
                "feedback_ratings", "overdue_goals", "checkpoint_progress",
                "feedback_sentiment", "skills_development", "peer_collaboration",
                "category_goal_success", "priority_goal_handling"
            ]
        
        elif template == ReportTemplateEnum.LEADERSHIP_FOCUS:
            return [
                "goal_completion", "feedback_ratings", "peer_collaboration",
                "category_goal_success", "training_completion"
            ]
        
        elif template == ReportTemplateEnum.TECHNICAL_FOCUS:
            return [
                "goal_completion", "skills_development", "training_completion",
                "checkpoint_progress", "category_goal_success"
            ]
    ...
    def _save_weekly_report(self, report: AIReportResponse):
        ...
        try:
            filename = f"report_{report.employee_id}_{datetime.now().strftime('%Y%m%d')}.txt"
    ...
    async def generate_team_summary_report(
        ...
        team_data = self._aggregate_team_data(db, team, team_members, period_start, period_end)
    ...
    def _aggregate_team_data(
        ...
        in_progress_goals = sum(1 for g in team_goals if g.status == GoalStatus.IN_PROGRESS)
        overdue_goals = sum(1 for g in team_goals if g.target_date < date.today() and g.status != GoalStatus.COMPLETED)
        
        goal_completion_rate = (completed_goals / total_goals * 100) if total_goals > 0 else 0
        
        # On-time rate
        completed_with_dates = [g for g in team_goals if g.status == GoalStatus.COMPLETED and g.completion_date]
        on_time = sum(1 for g in completed_with_dates if g.completion_date <= g.target_date)
        on_time_rate = (on_time / len(completed_with_dates) * 100) if completed_with_dates else 0
        
        # Team feedback
        team_feedback = db.query(Feedback).filter(
        ...
        avg_rating = sum(f.rating for f in rated_feedback) / len(rated_feedback) if rated_feedback else 0
        positive_feedback = sum(1 for f in team_feedback if f.feedback_type == "positive")
        positive_feedback_pct = (positive_feedback / total_feedback * 100) if total_feedback > 0 else 0
        
        # Team attendance
        team_attendance = db.query(Attendance).filter(
        ...
        present_records = sum(1 for a in team_attendance if a.status in [AttendanceStatus.PRESENT, AttendanceStatus.WFH])
        avg_attendance = (present_records / total_records * 100) if total_records > 0 else 0
        
        # Team training
        team_training = db.query(SkillModuleEnrollment).filter(
        ...
        # Team collaboration
        team_comments = db.query(GoalComment).join(Goal).filter(
            GoalComment.user_id.in_(member_ids),
            GoalComment.created_at >= datetime.combine(start_date, datetime.min.time()),
            GoalComment.created_at <= datetime.combine(end_date, datetime.max.time())
        ).count()
        
        blockers = db.query(GoalComment).join(Goal).filter(
            GoalComment.user_id.in_(member_ids),
        ...
        return {
            "team_name": team.name,
            "department_name": team.department.name if team.department else "Not assigned",
            ...
            "collaboration_score": "High" if team_comments > len(members) * 5 else "Moderate" if team_comments > len(members) * 2 else "Low"
        }
    ...
    def _get_member_summary(
        ...
        completed = sum(1 for g in member_goals if g.status == GoalStatus.COMPLETED)
        overdue = sum(1 for g in member_goals if g.target_date < date.today() and g.status != GoalStatus.COMPLETED)
        ...
        # Training
        training_completed = db.query(SkillModuleEnrollment).filter(
        ...
        if completion_rate >= 80:
            ...
            highlight = f"Strong learning commitment ({training_completed} modules completed)"
        else:
    ...
    async def generate_organization_report(
        ...
        org_data = self._aggregate_organization_data(db, departments, period_start, period_end)
        
        # Get department summaries
        department_summaries = []
        for dept in departments:
            dept_summary = self._get_department_summary(db, dept, period_start, period_end)
        ...
        report_markdown = await self.ai_provider.generate_report(prompt, max_tokens=3000)
    ...
    def _aggregate_organization_data(
        ...
        # Organization goals
        org_goals = db.query(Goal).filter(
        ...
        # On-time rate
        completed_with_dates = [g for g in org_goals if g.status == GoalStatus.COMPLETED and g.completion_date]
        on_time = sum(1 for g in completed_with_dates if g.completion_date <= g.target_date)
        on_time_rate = (on_time / len(completed_with_dates) * 100) if completed_with_dates else 0
        
        # Average overdue per employee
        overdue_goals = [g for g in org_goals if g.target_date < date.today() and g.status != GoalStatus.COMPLETED]
        avg_overdue = len(overdue_goals) / len(all_employees) if all_employees else 0
        
        # Organization feedback
        org_feedback = db.query(Feedback).filter(
        ...
        # Feedback frequency
        feedback_per_employee = total_feedback / len(all_employees) if all_employees else 0
        ...
        org_attendance = db.query(Attendance).filter(
        ...
        present = sum(1 for a in org_attendance if a.status in [AttendanceStatus.PRESENT, AttendanceStatus.WFH])
        attendance_rate = (present / total_att * 100) if total_att > 0 else 0
        
        # Training completion
        training = db.query(SkillModuleEnrollment).filter(
    ...
    def _get_department_summary(
        ...
        # Department goals
        dept_goals = db.query(Goal).filter(
            Goal.employee_id.in_(employee_ids),
            Goal.start_date >= start_date,
            Goal.start_date <= end_date,
            Goal.is_deleted == False
        ...
        # Department feedback
        dept_feedback = db.query(Feedback).filter(
            Feedback.employee_id.in_(employee_ids),
            Feedback.given_on >= datetime.combine(start_date, datetime.min.time()),
            Feedback.given_on <= datetime.combine(end_date, datetime.max.time()),
            Feedback.rating.isnot(None)
        ...
        avg_rating = sum(f.rating for f in dept_feedback) / len(dept_feedback) if dept_feedback else 0
        
        # Department attendance
        dept_attendance = db.query(Attendance).filter(
            Attendance.employee_id.in_(employee_ids),
            Attendance.date >= start_date,
            Attendance.date <= end_date
        ).all()
        ...
        present = sum(1 for a in dept_attendance if a.status in [AttendanceStatus.PRESENT, AttendanceStatus.WFH])
        attendance_rate = (present / total_att * 100) if total_att > 0 else 0
