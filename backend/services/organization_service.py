...
class OrganizationService:
    ...
    def get_manager_chain(db: Session, user_id: int) -> ManagerChainResponse:
        ...
        while current_user:
            ...
            if current_user.id in visited_ids:
                logger.warning(f"Circular reference detected in manager chain for user {user_id}")
                break
    ...
    @staticmethod
    def get_reporting_structure(db: Session, user_id: int) -> ReportingStructureResponse:
        """
        ...
        if user.manager_id:
            ...
            if manager and manager.manager_id:
                skip_level = db.query(User).filter(
                    User.id == manager.manager_id,
                    User.is_active == True
                ).first()
                if skip_level:
                    skip_level_manager = OrganizationService._format_user_node(skip_level, db)
        
        # Get direct reports (if user is a manager)
        direct_reports_users = db.query(User).filter(
            User.manager_id == user_id,
            User.is_active == True
        ).all()
        direct_reports = [OrganizationService._format_user_node(u, db) for u in direct_reports_users]
        
        # Get peers (same manager)
        peers = []
        if user.manager_id:
            peers_users = db.query(User).filter(
    ...
    @staticmethod
    def get_department_hierarchy(db: Session, department_id: int) -> DepartmentHierarchyResponse:
    ...
    def get_org_chart(db: Session, root_user_id: Optional[int] = None) -> OrgChartNode:
        ...
        if root_user_id:
            ...
            ).order_by(User.hierarchy_level).first()
    ...
    def _build_org_tree(user: User, db: Session, visited: set) -> OrgChartNode:
        ...
        if user.id in visited:
            return OrgChartNode(
                user=OrganizationService._format_user_node(user, db),
                children=[]
            )
    ...
    @staticmethod
    def _build_department_hierarchy(department: Department, db: Session) -> DepartmentHierarchyResponse:
        """Build department hierarchy with teams"""
        # Get department head
        head = None
        if department.head_id:
            head_user = db.query(User).filter(
                User.id == department.head_id,
                User.is_active == True
            ).first()
            if head_user:
        ...
        team_hierarchies = [
            OrganizationService._build_team_hierarchy(team, db)
            for team in teams
        ]
    ...
    def _build_team_hierarchy(team: Team, db: Session) -> TeamHierarchyResponse:
        ...
        # Get members
        members_users = db.query(User).filter(
