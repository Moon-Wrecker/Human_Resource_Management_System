...
from models import Application, JobListing, User, Department, ApplicationStatus
from pydantic_models import (
    CreateApplicationRequest, UpdateApplicationStatusRequest,
    ApplicationResponse, ApplicationFilters, ApplicationStatisticsResponse
)
...
class ApplicationService:
    ...
    def create_application(
        ...
        if applicant_user_id:
            existing_query = existing_query.filter(Application.applicant_id == applicant_user_id)
        else:
            existing_query = existing_query.filter(Application.applicant_email == request.applicant_email)
        ...
        logger.info(f"Application created: ID {new_application.id} for job {request.job_id}")
        
        return ApplicationService._map_application_to_response(new_application, include_details=True)
    ...
    def get_application_by_id(db: Session, application_id: int) -> ApplicationResponse:
        ...
        return ApplicationService._map_application_to_response(application, include_details=True)
    ...
    def update_application_status(
        ...
        if application.status != ApplicationStatus.PENDING and not application.reviewed_date:
            application.reviewed_date = datetime.utcnow()
        
        db.commit()
        db.refresh(application)
        
        logger.info(f"Application {application_id} status updated to {request.status.value} by user {reviewed_by_id}")
    ...
    @staticmethod
    def delete_application(db: Session, application_id: int, deleted_by_id: int) -> dict:
    ...
    def get_application_statistics(db: Session) -> ApplicationStatisticsResponse:
        ...
        reviewed = db.query(Application).filter(Application.status == ApplicationStatus.REVIEWED).count()
        ...
        top_jobs = db.query(
            ...
            Application, JobListing.id == Application.job_id
        ).group_by(
            JobListing.position
        ).order_by(
